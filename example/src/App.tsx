...
export default function App() {
  ...
  if (screen === 'bench')
    return <Bench variant={variant} mode={mode} run={run} onBack={() => setScreen('home')} />;

  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        <Text style={styles.title}>@obitrain/react-native-image</Text>
        <Button testID="goDemo" label="Demo (vertical slice)" onPress={() => setScreen('demo')} />
        <Text style={styles.section}>Bench variant</Text>
        <View style={styles.row}>
          {(['rn', 'fast', 'obi'] as BenchVariant[]).map((v) => (
            <Button key={v} testID={`variant-${v}`} label={v} active={variant === v} onPress={() => setVariant(v)} />
          ))}
        </View>
        <Text style={styles.section}>Bench mode</Text>
        <View style={styles.row}>
          {(['thumb', 'card', 'classic'] as BenchMode[]).map((m) => (
            <Button key={m} testID={`mode-${m}`} label={m} active={mode === m} onPress={() => setMode(m)} />
          ))}
        </View>
        <Button testID="goBench" label={`Run bench: ${variant} / ${mode}`} onPress={() => { setRun((r) => r + 1); setScreen('bench'); }} />
      </ScrollView>
    </SafeAreaView>
  );
}
...
export function Button({
  label, onPress, testID, active,
}: { label: string; onPress: () => void; testID?: string; active?: boolean }) {
  return (
    <Pressable testID={testID} onPress={onPress} style={[styles.button, active && styles.buttonActive]} accessibilityRole="button">
...
const styles = StyleSheet.create({
  ...
  button: { paddingVertical: 12, paddingHorizontal: 16, borderRadius: 8, backgroundColor: '#eee', alignItems: 'center' },
